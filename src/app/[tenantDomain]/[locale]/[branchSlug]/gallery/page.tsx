...
async function resolveDynamicItems(
  ...
  try {
    ...
    console.error('[gallery] fetchMenu failed:', err);
    return new Map();
  }
...
export default async function GalleryPage({
  ...
  if (!tenant.features?.hasGallery) {
    return <div className="min-h-screen flex items-center justify-center"><h1 className="text-2xl">Gallery not available</h1></div>;
  }
  ...
  try {
    ...
    console.error('[gallery] fetchBranchBySlug failed:', err);
  }
  ...
  try {
    sections = await fetchPublicBranchSections(tenant.tenantId, branchId, 'gallery');
  } catch (err) {
    console.error('[gallery] fetchPublicBranchSections failed:', err);
  }

  // If no sections configured, show fallback
  if (!sections || sections.length === 0) {
    return (
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-3xl font-bold mb-4">Gallery</h1>
          <p className="text-muted-foreground">No gallery content available yet.</p>
        </div>
      </section>
    );
  }
